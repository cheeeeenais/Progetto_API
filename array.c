                    ...
                    while (dest_node_index < d) { // horizontal cycle
                        ...
                        if (weight != 0) {
                            ...
                            if (src_node_index == 0) { // if STARTING NODE == 0 -> initializing the HEAP
                                ...
                                //if (min_heap[0].dest == dest_node_index && min_heap[0].weight < weight) don't insert
                                //if (min_heap[0].dest != dest_node_index || min_heap[0].weight >= weight) insert
            ...
            while (heap_size > 0 && d_count < d - 1 && finish == 'f') { // calculating the min_paths
                    ...
                    if (graph_support == graph_starting_array[dest_to_remember]->to_be_modified &&
                        ...
                        while (graph_support != NULL && finish == 'f') {
                            if (graph_starting_array[graph_support->dest]->min_path == INFINITY &&
                                d_count < d - 1) {
                                ...
                                /*else if (heap_size == 0 && k_count >= k && max != NULL) {
                                    if (weight_to_remember +
                                        graph_support->weight > max->sum) {
                                        finish = 't';
                                        sum = INFINITY;
                                    }
